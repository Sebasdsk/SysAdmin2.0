...
function Show-SshMenu {
    while ($true) {
        ...
        Write-Host "2. Validar Estado y Conexion"
...
function Install-SshServer {
    Write-Host "--- Instalacion y Configuracion de OpenSSH ---" 
    ...
    if ($sshCheck.State -ne 'Installed') {
        Write-Host "Instalando caracteristica OpenSSH Server (puede tardar un momento)..." 
...
function Validate-Ssh {
    ...
    Write-Host "--- Informacion de Conexion ---" 
    
    # Reutilizamos la funcion Get-InternalInterface de Common.ps1 para dar la IP correcta de la VM
    ...
    if ($nic) {
        ...
        Write-Host "No se pudo determinar la IP de la red interna automaticamente."
    }
    
    # Comprobar que el puerto realmente esta "Listening" (Escuchando)
    $portCheck = Get-NetTCPConnection -LocalPort 22 -State Listen -ErrorAction SilentlyContinue
    if ($portCheck) {
        Write-Host "`nEl servidor esta escuchando activamente en el puerto 22." 
    } else {
